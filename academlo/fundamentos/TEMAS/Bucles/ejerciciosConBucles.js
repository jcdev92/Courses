...
function numbersArr(number1, number2) {
  let arrNums = [];
  for (number1; number2 >= number1; number1++) {
      arrNums.push(number1)
  }
  return arrNums
}
// 
console.log(numbersArr(12,30))
...
console.log(nsum([5,8,12,20]));
console.log(nsum(arrayNumbers));
...
function middlearr(arreglo) {
  max_index = arreglo.length-1;
  middle = max_index/2
  if (Number.isInteger(middle)) {
    return [arreglo[middle]]
  } else {
    return [arreglo[middle-0.5], arreglo[middle+0.5]]
  }
}
console.log(middlearr([8,5,6,10,3,9,4,2,1,0]))
console.log(middlearr([1, 3, 4]))
